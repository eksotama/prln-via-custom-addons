        ...
        for _obj in self.pool.get(_model).browse(cr, uid, select, context=context):
            ...
            if not _tax_inv:
                ...
                if isinstance(ids, (int, long, )) or (len(ids) == 1):
                    raise osv.except_osv(_('Error!'), _("No Invoice is specified"))
            elif _tax_inv.state not in ('open', 'paid'):
                # Invoice is not in 'open' or 'paid' state, so don't create taxform
                res[_obj.id] = False
                if isinstance(ids, (int, long, )) or (len(ids) == 1):
                    raise osv.except_osv(_('Error!'), _("Invoice %s is not in 'open' or 'paid' state.") % (_obj.internal_number))
            elif _tax_inv.type in ('out_refund', 'in_invoice'):
                # Taxform cannot be generated for customer refund or supplier invoice
                res[_obj.id] = False
                if isinstance(ids, (int, long, )) or (len(ids) == 1):
                    raise osv.except_osv(_('Error!'), _('Taxform cannot be generated for customer refund or supplier invoice.'))
                ...
                if _obj.taxform_id.state in ['printed', 'draft']:
                    ...
                    if isinstance(ids, (int, long, )) or (len(ids) == 1):
                        raise osv.except_osv(_('Warning!'), _('Taxform has been generated for invoice %s.') % (_obj.internal_number))
            else:
