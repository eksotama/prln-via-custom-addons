...
class lot_valuation(osv.osv):
    ...
    _columns = {
        ...
        'lot_id': fields.many2one('stock.production.lot', 'Lot No', required=True,
    ...
    def check_accounts(self, cr, uid, ids, context=None):
        ...
        if product.valuation != 'real_time':
            warning_message += "Valuation can't be performed because valuation in product is not Real Time(automated)"
            warn_flag = False
        ...
        if not user.company_id.valuation_journal_id:
            warning_message += "\n" + "Valuation can't be performed because Valuation Journal is not configured in the Company!"
            warn_flag = False
        if not warn_flag:
