...
class via_prorate_discount(osv.osv_memory):
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        ...
        if (len(context.get('active_ids')) != 1) or (_po.state not in ['draft']):
            raise osv.except_osv(_('Error !'), _('You can only Prorate Discount for 1 Request for Quotation.'))
    ...
    def do_apply_discount(self, cr, uid, ids, context=None):
        ...
        if _po:
            ...
            for line in _po.order_line:
                _po_line_pool.write(cr, uid, [line.id], {'price_unit': line.price_unit * (100 - _disc_percent) / 100}, context=context)
